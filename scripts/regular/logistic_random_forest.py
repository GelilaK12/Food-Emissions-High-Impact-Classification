...
IMAGE_FOLDER = os.path.join(BASE_DIR, "images", "logistic_random_forest")
...
def log_model_run(model, model_name, X_test, y_test, artifacts_dict, params_dict):
       ...
       with mlflow.start_run(run_name=model_name):
        ...
        for csv_file in artifacts_dict.get("csv", []):
            mlflow.log_artifact(csv_file)
        
        for img_file in artifacts_dict.get("images", []):
...
# MLflow logging for Logistic Regression
log_model_run(
    log_model,
    "logistic_regression",
    X_test,
    ...
    artifacts_dict={
        "csv": [
            f"{OUTPUT_FOLDER}/logistic_regression_classification_report.csv",
            f"{OUTPUT_FOLDER}/logistic_regression_confusion_matrix.csv",
            f"{OUTPUT_FOLDER}/logistic_regression_coefficients.csv",
        ],
        "images": [f"{IMAGE_FOLDER}/logistic_regression_coefficients.png"],
    },
    params_dict={"model_type": "logistic_regression", "solver": log_model.solver, "max_iter": log_model.max_iter},
)
...
log_model_run(
    ...
    artifacts_dict={
        "csv": [
            f"{OUTPUT_FOLDER}/random_forest_classification_report.csv",
            f"{OUTPUT_FOLDER}/random_forest_confusion_matrix.csv",
            f"{OUTPUT_FOLDER}/random_forest_feature_importances.csv",
            f"{OUTPUT_FOLDER}/random_forest_permutation_importance.csv",
            f"{OUTPUT_FOLDER}/feature_correlations.csv",
        ],
        "images": [
            f"{IMAGE_FOLDER}/random_forest_feature_importances.png",
            f"{IMAGE_FOLDER}/farm_by_class.png",
            f"{IMAGE_FOLDER}/land_use_change_by_class.png",
        ],
    },
    params_dict={"model_type": "random_forest", "n_estimators": rf_model.n_estimators, "max_depth": rf_model.max_depth},
)