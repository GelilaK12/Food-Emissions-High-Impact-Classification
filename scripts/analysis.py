...
pd.DataFrame(log_report).transpose().to_csv("outputs/logistic_regression_classification_report.csv")
log_cm = confusion_matrix(y_test, y_pred_log)

pd.DataFrame(log_cm,index=["Actual 0", "Actual 1"],columns=["Predicted 0", "Predicted 1"]).to_csv("outputs/logistic_regression_confusion_matrix.csv")

# Coefficient interpretation (directional influence on odds)
...
plt.tight_layout()
plt.savefig("images/logistic_regression_coefficients.png")
plt.close()
...
pd.DataFrame(rf_report).transpose().to_csv("outputs/random_forest_classification_report.csv")

rf_cm = confusion_matrix(y_test, y_pred_rf)
pd.DataFrame(rf_cm,index=["Actual 0", "Actual 1"], columns=["Predicted 0", "Predicted 1"]).to_csv("outputs/random_forest_confusion_matrix.csv")


# Feature importance (impurity-based)
rf_importances = pd.Series(rf_model.feature_importances_,index=features).sort_values(ascending=False)

rf_importances.to_csv("outputs/random_forest_feature_importances.csv")
...
corr_df.corr()["High_Impact"].sort_values(ascending=False).to_csv("outputs/feature_correlations.csv")
...
plt.tight_layout()
plt.savefig("images/farm_by_class.png")
plt.close()
...
plt.tight_layout()
plt.savefig("images/land_use_change_by_class.png")
plt.close()
...
perm_importances.to_csv("outputs/random_forest_permutation_importance.csv")
